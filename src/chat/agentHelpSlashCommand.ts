...
async function agentHelpHandler(agentSlashCommandsOwner: SlashCommandsOwner, request: AgentRequest): Promise<SlashCommandHandlerResult> {
    const slashCommandsMarkdown = Array.from((await agentSlashCommandsOwner.getSlashCommands()).entries()).map(([name, config]) => `- \`/${name}\` - ${config.longDescription || config.shortDescription}`).join("\n");
